...
trap 'set +e; restore_dist_files "${yaml_files[@]}"; pdsh -R ssh -S -w ${HOSTPREFIX}vm[1-8] "sudo umount /mnt/daos; x=0; while [ \$x -lt 30 ] && grep $DAOS_BASE /proc/mounts && ! sudo umount $DAOS_BASE; do sleep 1; let x=\$x+1; done; sudo rmdir $DAOS_BASE" 2>&1 | dshbak -c' EXIT
